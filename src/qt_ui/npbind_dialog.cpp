...
void NpBindDialog::updateHexView() {
    ...
    // Update details
    m_lblType->setText(QString("0x%1").arg(static_cast<quint16>(body.npcommid.type), 2, 16, QChar('0')).toUpper());
...
void NpBindDialog::onExportJson() {
    ...
    QJsonObject root;
    QJsonObject hdr;
    hdr["magic"] = QString("0x%1").arg(static_cast<quint32>(m_npfile.Header().magic), 8, 16, QChar('0'));
