...
ControlSettings::ControlSettings(std::vector<game_info> game_info,
    ...
    : QDialog(parent), m_emu_settings(std::move(emu_settings)), m_game_info(game_info),
      m_ipc_client(ipc_client), GameRunning(isGameRunning), RunningGameSerial(GameRunningSerial),
    ...
    ui->PerGameCheckBox->setChecked(!m_emu_settings->IsUseUnifiedInputConfig());
    ...
    connect(ui->DefaultGamepadButton, &QPushButton::clicked, this, [this]() {
        ...
        m_emu_settings->SetDefaultControllerId(GUID);
        m_emu_settings->Save();
    ...
    connect(ui->RemoveDefaultGamepadButton, &QPushButton::clicked, this, [this]() {
        ...
        m_emu_settings->SetDefaultControllerId("");
        m_emu_settings->Save();
...
void ControlSettings::SaveControllerConfig(bool CloseOnSave) {
    ...
    if (GameRunning) {
        m_emu_settings->IsUseUnifiedInputConfig() ? m_ipc_client->reloadInputs("default")
...
void ControlSettings::AddBoxItems() {
    ...
    for (int i = 0; i < m_game_info.size(); i++) {
        ui->ProfileComboBox->addItem(QString::fromStdString(m_game_info[i]->info.serial));
    }
...
void ControlSettings::GetGameTitle() {
    if (ui->ProfileComboBox->currentText() == tr("Common Config")) {
        ...
        for (int i = 0; i < m_game_info.size(); i++) {
            if (m_game_info[i]->info.serial == ui->ProfileComboBox->currentText().toStdString()) {
                ui->TitleLabel->setText(QString::fromStdString(m_game_info[i]->info.name));
            }
        }
    }
}
...
void ControlSettings::CheckGamePad() {
    ...
    int defaultIndex = GamepadSelect::GetIndexfromGUID(gamepads, gamepad_count,
                                                       m_emu_settings->GetDefaultControllerId());
